...
(function() {
  ...
  function extractElementsToInject(type) {
    ...
    document
      .querySelectorAll(`[${DATA_ATTRIBUTE_NAMES.dataCode}][${DATA_ATTRIBUTE_NAMES.type}=${type}]`)
      .forEach(function(element) {
        if (type === 'script') {
          elementsToInject.push(element.getAttribute(DATA_ATTRIBUTE_NAMES.dataCode));
        } else {
          elementsToInject.push({
            html: element.getAttribute(DATA_ATTRIBUTE_NAMES.dataCode),
  ...
  function extractCurrentCofig() {
    ...
    types.forEach(function(type) {
      ...
      document
        .querySelectorAll(
          `[${DATA_ATTRIBUTE_NAMES.dataCode}][${DATA_ATTRIBUTE_NAMES.type}="${type}"]`
        )
        .forEach(function(element) {
  ...
  function extractInfoFromElement(element) {
    ...
    info.code = element.getAttribute(DATA_ATTRIBUTE_NAMES.dataCode);
  ...
  function createActions(row) {
    ...
    let editAction = createActionButton(
      ACTIONS_LABELS.edit,
      function() {
        let element = row.querySelector(`[${DATA_ATTRIBUTE_NAMES.dataCode}]`);
        let info = extractInfoFromElement(element);
        fillEditor(info);
  ...
  function updateInfoRow(row, info) {
    let infoContainer = row.querySelector(`[${DATA_ATTRIBUTE_NAMES.dataCode}]`);
  ...
  function updateInfoInContainer(container, info) {
    container.innerHTML = info.verbose;
    container.setAttribute(DATA_ATTRIBUTE_NAMES.dataCode, info.code);
  ...
  function finishAdditionFromEditor() {
    ...
    if (currentAction === ACTIONS.addSrcipt) {
  ...
  function addListeners() {
    ...
    document
      .getElementById(ELEMENTS_IDS.addComponentBtn)
      .addEventListener('click', addNewComponent);
